              ...
              #  query_lis = range(num_queries)
               ...
               # for i_idx in query_lis:
                ...
                for i_idx in range(num_queries):
                    ...
                    a[i_idx + c_idx * num_queries][t_idx] = self.transform(class_data_list[num_shots + i_idx]) 
                    b[i_idx + c_idx * num_queries][t_idx] = c_idx
            '''
            shuffle_ix = np.random.permutation(num_ways*num_queries)
            train_data = np.array(query_data)[shuffle_ix][t_idx]
            train_label = np.array(query_label)[shuffle_ix][t_idx]
            print(train_label)
            '''
            query_lis = list(range(num_ways*num_queries))
            random.shuffle(query_lis)
               
            for i,k in enumerate(query_lis): 
                a[i][t_idx] = query_data[k][t_idx]
            #    print(query_label[k][t_idx])
                b[i][t_idx] = query_label[k][t_idx]
             #   print(b[i][t_idx])
