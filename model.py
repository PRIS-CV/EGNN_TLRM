...
class NodeUpdateNetwork(nn.Module):
    ...
    #    edge_feat1 = F.normalize(self.att * diag_mask, p=1, dim=-1)
...
class EdgeUpdateNetwork(nn.Module):
    ...
    def forward(self, node_feat, edge_feat):
        ...
        '''
        d = node_feat.bmm(node_feat.transpose(1,2))
        d = d/math.sqrt(self.in_features)
        d = F.softmax(d, dim= -1)
        num_tasks = node_feat.size(0)
        num_data = node_feat.size(1)
        for i in range (num_data):
            d_edge_feat = torch.abs(d[:,:,i].unsqueeze(-1)* node_feat)
            d_edge_feat = d_edge_feat.unsqueeze(1)

            node_feat1 = torch.cat([node_feat1, d_edge_feat],1) if i>0 else d_edge_feat
        x_ij = torch.transpose(node_feat1, 1, 3)
        # compute similarity/dissimilarity (batch_size x feat_size x num_samples x num_samples)
        sim_val = F.sigmoid(self.sim_network(x_ij))
        if self.separate_dissimilarity:
            dsim_val = F.sigmoid(self.dsim_network(x_ij))
